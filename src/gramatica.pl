...
sintagma_verbal(S0, S):- verbo(S0,S1), pronombre_relativo(S1, S2), verbo(S2, S3), preposicion(S3,S).
sintagma_verbal(S0, S):- verbo(S0,S1), pronombre_relativo(S1, S2), verbo(S2, S).
...
sintagma_nominal(S0, S):- pronombre(S0,S1), pronombre_reflexivo(S1, S).
